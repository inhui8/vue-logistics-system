  ...
  <div class="delivery-trains-container">
    ...
    <div class="filter-bar">
      <div class="filter-row">
        ...
        <el-select v-model="createDateType" placeholder="创建日期" size="small" class="filter-item">
          ...
          <el-option label="出单日期" value="issue"></el-option>
          <el-option label="装车预约时间" value="appointment"></el-option>
        </el-select>
        ...
        <!-- 批次号 -->
        <el-select v-model="searchType" placeholder="批次号" size="small" class="filter-item">
          <el-option label="车次号" value="trainNumber"></el-option>
          <el-option label="PC号" value="trainNumber"></el-option>
    ...
    <div class="view-management">
      ...
      <el-radio-group v-model="currentView" size="small" class="status-view-group">
        ...
        <el-radio-button value="waitRegister">待登记(107)</el-radio-button>
        <el-radio-button value="registered">已登记(152)</el-radio-button>
        <el-radio-button value="loading">装车中(0)</el-radio-button>
        <el-radio-button value="loaded">装车完成(3)</el-radio-button>
        <el-radio-button value="shipped">已发货(1585)</el-radio-button>
        <el-radio-button value="delivered">已签收(16351)</el-radio-button>
        <el-radio-button value="pod">已签收POD(3585)</el-radio-button>
        <!-- 动态添加自定义视图 -->
        <el-radio-button 
      ...
      >
              ...
              >
                ...
                >
                  <template #default="scope">
                    <template v-if="column.prop === 'expenseType'">
                      <el-select 
        ...
        <template v-for="column in tableColumns" :key="column.prop">
          ...
          >
            <template #default="scope">
              <!-- 可编辑单元格 -->
              <div v-if="['loadingType', 'deliveryType', 'carNumber', 'loader', 'dockNumber', 'warehouseRemarks', 
                'driverLicense', 'licensePlate', 'driverPhone', 'supplier', 'actualPallets', 
                'palletsMerged', 'deliveryCost'].includes(column.prop) && 
                editingCell.rowIndex === scope.$index && 
                editingCell.prop === column.prop" 
                class="cell-editor">
                
                <el-select 
                  v-if="['loadingType', 'deliveryType', 'loader', 'dockNumber', 'driverLicense', 'licensePlate', 'driverPhone', 'supplier'].includes(column.prop)"
                ...
                <el-input 
                  ...
                  @blur="saveEdit(scope.row, column.prop)"
                  @keyup.enter="saveEdit(scope.row, column.prop)"
                  :type="['actualPallets', 'palletsMerged', 'deliveryCost'].includes(column.prop) ? 'number' : 'text'"
                ></el-input>
              </div>
              
              <!-- 普通单元格，带编辑标记 -->
              <div 
                v-else-if="['loadingType', 'deliveryType', 'carNumber', 'loader', 'dockNumber', 'warehouseRemarks', 
                  'driverLicense', 'licensePlate', 'driverPhone', 'supplier', 'actualPallets', 
                  'palletsMerged', 'deliveryCost'].includes(column.prop)"
                class="editable-cell"
    ...
    <!-- 文件上传带标签对话框 -->
    <file-upload-with-tag-dialog
      v-model:visible="fileUploadWithTagVisible"
    ...
    <el-tabs v-model="activeTab" class="delivery-tabs">
      <el-tab-pane label="全部" name="all">
        <div class="badge-container">
          <el-badge :value="pendingPODCount" :hidden="pendingPODCount === 0" type="warning">
            <span>POD待审核</span>
          </el-badge>
          <el-badge :value="pendingFileCount" :hidden="pendingFileCount === 0" type="warning">
            <span>文件待审核</span>
          </el-badge>
        </div>
      </el-tab-pane>
...
// 使用导入的JSON数据
const defaultData = deliveryTrainsData;
...
export default {
  ...
  setup() {
    ...
    const handleDetail = (row) => {
      ...
      // 跳转到详情页面
      router.push(`/tms/delivery-train-detail/${row.id}`);
    };
    ...
    const handleCellClick = (row, column, cell) => {
      ...
      // 只允许编辑特定字段
      if (['loadingType', 'deliveryType', 'carNumber', 'loader', 'dockNumber', 'warehouseRemarks', 
           'driverLicense', 'licensePlate', 'driverPhone', 'supplier', 'actualPallets', 
           'palletsMerged', 'deliveryCost'].includes(prop)) {
    ...
    const pendingPODCount = computed(() => {
      // 这里需要根据实际数据计算待审核的POD数量
      return 2; // 示例数据
    });

    const pendingFileCount = computed(() => {
      // 这里需要根据实际数据计算待审核的文件数量
      return 1; // 示例数据
    });
    ...
    const expenseTypes = ref([
      ...
      { value: 'abnormal', label: '异常费' },
      { value: 'waiting', label: '等时费' },
      { value: 'loading', label: '装车费' },
      { value: 'other', label: '其他费用' }
    ]);

    // 获取费用类型选项
    const getExpenseTypeOptions = () => {
      return expenseTypes.value;
    };

    // 在 setup 函数中添加 handleExpenseTypeChange 方法
    const handleExpenseTypeChange = (row) => {
      console.log('费用类型已更改:', row.expenseType);
