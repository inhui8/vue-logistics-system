  ...
  <div class="delivery-trains-container">
    ...
    <div class="view-management">
      ...
      <el-radio-group v-model="currentView" size="small" class="status-view-group">
        ...
        <el-radio-button value="waitRegister">已备货(107)</el-radio-button>
        <el-radio-button value="registered">已预约(152)</el-radio-button>
        <el-radio-button value="loading">已check in(0)</el-radio-button>
        <el-radio-button value="loaded">已提走(3)</el-radio-button>
        <el-radio-button value="shipped">已送达(1585)</el-radio-button>
        <el-radio-button value="delivered">已发POD(16351)</el-radio-button>
        <!-- 动态添加自定义视图 -->
        <el-radio-button 
    ...
    <div class="table-container">
      <groupable-table
        ...
        :resizable="true"
      >
        <!-- Expandable Column - Child Table -->
        <el-table-column type="expand">
          <template #default="props">
            <div class="sub-table-container">
              <h4 class="sub-table-title">PC信息</h4>
              <el-table
              ...
              >
                <el-table-column
                  ...
                  :fixed="column.fixed"
                >
                  <template #default="scope" v-if="column.prop === 'podImage'">
                    <el-image
                      v-if="scope.row.podImage"
                      :src="scope.row.podImage"
                      :preview-src-list="[scope.row.podImage]"
                      fit="cover"
          ...
          <el-timeline>
            ...
            >
              <div class="follow-up-item">
                ...
                <div class="follow-up-files" v-if="record.files && record.files.length">
                  <div class="file-list">
    ...
    >
          ...
          <el-form :model="expenseForm" label-width="100px">
            <el-form-item label="费用类型" required>
              <el-select v-model="expenseForm.expenseType" placeholder="请选择费用类型" style="width: 100%">
                ...
                <el-option label="装卸费" value="loading"></el-option>
                <el-option label="仓储费" value="storage"></el-option>
            ...
            <el-form-item label="币种" required>
              <el-select v-model="expenseForm.currency" placeholder="请选择币种" style="width: 200px">
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="expenseDetailVisible = false">取消</el-button>
          <el-button type="primary" @click="handleAddExpense">添加</el-button>
        </span>
      </template>
    </el-dialog>
    ...
    <el-dialog
      ...
      width="500px"
    >
      <el-form :model="editExpenseForm" label-width="100px">
        <el-form-item label="费用类型" required>
          <el-select v-model="editExpenseForm.expenseType" placeholder="请选择费用类型" style="width: 100%">
            <el-option label="运输费" value="transport"></el-option>
            <el-option label="装卸费" value="loading"></el-option>
            <el-option label="仓储费" value="storage"></el-option>
            <el-option label="其他费用" value="other"></el-option>
        ...
        <el-form-item label="币种" required>
          <el-select v-model="editExpenseForm.currency" placeholder="请选择币种" style="width: 200px">
            <el-option label="人民币" value="CNY"></el-option>
            <el-option label="美元" value="USD"></el-option>
            <el-option label="欧元" value="EUR"></el-option>
          </el-select>
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="editExpenseVisible = false">取消</el-button>
          <el-button type="primary" @click="saveEditExpense">保存</el-button>
        </span>
      </template>
    </el-dialog>
...
import { DArrowLeft, PictureFilled, Document } from '@element-plus/icons-vue';
import draggable from 'vuedraggable';

// 定义表格列
const defaultTableColumns = supplierDeliveryOrdersColumns;

// 使用导入的JSON数据
const defaultData = supplierDeliveryOrdersData;
...
export default {
  ...
  setup() {
    ...
    const childColumns = ref(childTableColumns);
    ...
    const totalItems = ref(supplierDeliveryOrdersData.length);
    ...
    const fetchData = () => {
      ...
      try {
        // 直接使用导入的数据数组
        tableData.value = supplierDeliveryOrdersData;
        totalItems.value = supplierDeliveryOrdersData.length;
        applyFiltersAndPagination();
      } catch (error) {
    ...
    const handleCommand = (command, row) => {
      switch (command) {
        ...
        case 'fileUploadWithTag':
          ...
          break;
        case 'expenseDetail':
          handleExpenseDetail(row);
          break;
        case 'expensePull':
          handleExpensePull(row);
          break;
      }
    };
    ...
    const handleExpandChange = (row, expanded) => {
      ...
      if (expanded) {
        // 确保只有当前行被展开
        expandedRowKeys.value = [row.orderNumber];
      } else {
        // 如果是折叠操作,清空展开行
    ...
    const saveFollowUp = () => {
      ...
      followUpHistory.value.unshift({
        ...
        files: followUpForm.value.files.map(file => ({
          name: file.name,
    ...
    const handleViewDetails = (row) => {
      console.log('查看PC详情:', row);
      // 实现查看详情的逻辑
    };

    const handleUploadPod = (row) => {
      console.log('上传POD:', row);
      // 实现上传POD的逻辑
    };

    const handleDeletePc = (row) => {
      ElMessageBox.confirm('确认要删除该PC记录吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        // 实现删除PC的逻辑
        console.log('删除PC:', row);
        ElMessage.success('删除成功');
      }).catch(() => {
        // 取消删除
      });
    ...
    // 在 setup 函数中添加
    const uploadUrl = ref('/api/upload'); // 替换为实际的上传接口地址

    // 处理文件上传成功
    const handleUploadSuccess = (response, file, fileList) => {
      followUpForm.value.files = fileList;
      ElMessage.success('文件上传成功');
    };
    ...
    const handleAddExpense = () => {
      if (!expenseForm.value.expenseType || !expenseForm.value.amount || !expenseForm.value.currency) {
    ...
    const handleEditExpense = (row) => {
      editExpenseForm.value = { ...row };
      editExpenseVisible.value = true;
    };

    // 保存编辑的费用
    const saveEditExpense = () => {
      if (!editExpenseForm.value.expenseType || !editExpenseForm.value.amount || !editExpenseForm.value.currency) {
        ElMessage.warning('请填写完整的费用信息');
        return;
      }

      // 这里应该调用API更新费用
      const index = expenseList.value.findIndex(item => item.id === editExpenseForm.value.id);
      if (index !== -1) {
        expenseList.value[index] = { ...editExpenseForm.value };
      }
    ...
    const handleDeleteExpense = (row) => {
      ElMessageBox.confirm('确认要删除该费用记录吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
    ...
    // 添加费用拉取的处理方法
    const handleExpensePull = (row) => {
      ElMessageBox.confirm('确认要拉取该订单的费用明细吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        // 这里应该调用API拉取费用
        ElMessage.success('费用拉取成功');
        // 拉取成功后刷新费用明细
        handleExpenseDetail(row);
      }).catch(() => {
        // 取消拉取
