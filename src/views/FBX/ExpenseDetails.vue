...
<!-- eslint-disable vue/valid-next-tick -->
...
<template>
    <div class="delivery-trains-container">
      ...
      <div class="table-container">
        ...
        >
          ...
          <el-table-column type="expand">
            <template #default="props">
              <div class="detail-container">
                <!-- 基础资料区域 -->
                <el-descriptions title="基础信息" :column="4" border class="basic-info-section">
                  <el-descriptions-item label="订单类型">{{ props.row.orderType || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="产品渠道">{{ props.row.channel || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="运输路线">{{ props.row.route || 'T2' }}</el-descriptions-item>
                  <el-descriptions-item label="客户名称">{{ props.row.customerName || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="目的地">{{ props.row.destination || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="ETD">{{ props.row.etd || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="船名">{{ props.row.shipName || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="航次">{{ props.row.voyage || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="ETA">{{ props.row.eta || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="柜号">{{ props.row.containerNumber || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="40HQ">{{ props.row.containerSize || '40HQ' }}</el-descriptions-item>
                  <el-descriptions-item label="MBL/SO/AWB">{{ props.row.mbl || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="柜号">{{ props.row.containerNumberAgain || props.row.containerNumber || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="提单号">{{ props.row.blNumber || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="封条号">{{ props.row.sealNumber || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="清关方式">{{ props.row.customsClearance || '代理清关' }}</el-descriptions-item>
                  <el-descriptions-item label="代理报关">{{ props.row.customsBroker || '自理清关' }}</el-descriptions-item>
                  <el-descriptions-item label="提柜码头">{{ props.row.pickupTerminal || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="暂无对应架">{{ props.row.noFrame || '无' }}</el-descriptions-item>
                  <el-descriptions-item label="入仓仓库">{{ props.row.warehouseCode || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="代理提柜">{{ props.row.pickupAgent || '代理提柜' }}</el-descriptions-item>
                  <el-descriptions-item label="自有卡车">{{ props.row.ownTruck || '自有卡车' }}</el-descriptions-item>
                  <el-descriptions-item label="重返码头">{{ props.row.returnTerminal || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="有无机架">{{ props.row.hasFrame || '无' }}</el-descriptions-item>
                  <!-- 节点跟踪 -->
                  <el-descriptions-item label="海关放行时间" :span="1">{{ props.row.customsReleaseTime || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="海关暂扣时间" :span="1">{{ props.row.customsHoldTime || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="卸船时间" :span="1">{{ props.row.dischargeTime || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="AVAILABLE 时间" :span="1">{{ props.row.availableTime || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="预计提柜时间" :span="1">{{ props.row.expectedPickupTime || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="提柜时间" :span="1">{{ props.row.containerPickupTime || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="还柜时间" :span="1">{{ props.row.returnTime || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="车牌输入人" :span="1">{{ props.row.plateInputUser || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="港口免柜天数" :span="1">{{ props.row.freeDaysPort || '0' }}</el-descriptions-item>
                  <el-descriptions-item label="预计拆柜时间" :span="1">{{ props.row.expectedUnpackingTime || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="实际拆柜时间" :span="1">{{ props.row.actualUnpackingTime || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="预计备检时间" :span="1">{{ props.row.expectedInspectionTime || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="完成拆柜时间" :span="1">{{ props.row.unpackingCompletedTime || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="入仓仓库" :span="1">{{ props.row.warehouseCodeAgain || props.row.warehouseCode || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="韧约号" :span="1">{{ props.row.appointmentRef || '-' }}</el-descriptions-item>
                  <el-descriptions-item label="APPT" :span="1">{{ props.row.appt || '-' }}</el-descriptions-item>
                </el-descriptions>

                <!-- 标签页 -->
                <el-tabs type="border-card" class="detail-tabs">
                  <el-tab-pane label="派送计划">
                  ...
                  <el-tab-pane label="文件管理">
                    <div class="placeholder-content">文件管理内容待添加</div>
                  </el-tab-pane>
                  <el-tab-pane label="操作日志">
                     <div class="placeholder-content">
                        <el-timeline style="padding-left: 10px; padding-top: 10px;">
                          <el-timeline-item
                            v-for="(record, index) in (props.row.history || [])"
                            :key="index"
                          ...
                          <el-timeline-item v-if="!(props.row.history && props.row.history.length > 0)">
                            暂无操作日志
                          </el-timeline-item>
                        </el-timeline>
                     </div>
                  ...
                  <el-tab-pane label="费用明细">
                    <div class="placeholder-content">
                     费用明细已移至独立页面。
                     <el-button type="primary" link @click="goToExpenses(props.row.id)" style="margin-left: 10px;">
                       点击跳转
                     </el-button>
                   </div>
                  </el-tab-pane>
          ...
          <!-- 分组信息列 (如果启用了分组) -->
          <el-table-column v-if="groupField" label="分组信息" width="200" fixed="left">
          ...
          <!-- 动态生成主表格列 - 减少列数 -->
          <template v-for="column in visibleMainTableColumns" :key="column.prop">
            <el-table-column
              v-if="column.visible !== false && column.prop === 'appointmentStatus'"
              :prop="column.prop"
              :label="column.label"
            ...
            >
              <template #default="scope">
                 <!-- 仅显示非分组行的数据 -->
                 <!-- 移除所有之前的编辑逻辑 -->
                <span v-if="!scope.row.isGroup">{{ scope.row[column.prop] }}</span>
              </template>
            </el-table-column>
          </template>
        </groupable-table>
  ...
  export default {
    ...
    setup() {
      // 使用新的容器发货数据源（不在全局作用域声明）
      const tableColumns = ref(containerShipmentsColumns);
      const childColumns = ref(containerItemsColumns);
      const tableData = ref(containerShipmentsData);
      const displayData = ref([]);
      const loading = ref(false);
      const visibleColumns = ref([...containerShipmentsColumns]);
      ...
      const handleCommand = (command, row) => {
        ...
        switch (command) {
          ...
          case 'palletDetail':
            ...
            // 可以在这里导航到新的页面或打开一个模态框
            // router.push({ name: 'PalletDetail', params: { id: row.id } });
            break;
          case 'copyAddress':
            // 复制地址信息到剪贴板
      ...
      const removeTag = (row, tagToRemove) => {
        if (row.tags) {
          if (typeof row.tags === 'string' && currentRow.value.tags) {
      ...
      const visibleMainTableColumns = computed(() => {
        return tableColumns.value.filter(col => [
          'containerNumber', // 保留关键标识
          'shippingNumber',
          'appointmentStatus', // 保留状态
          'tags', // 保留标签
          'eta', // 保留关键日期
          'operation' // 保留操作列
        ].includes(col.prop) && col.visible !== false); // 确保列本身是可见的
      });

      // 在详情页跳转费用的辅助函数 (如果需要从详情页跳转)
      const goToExpenses = (id) => {
        router.push({ name: 'FBXExpenseDetails', params: { id } });
      };
      ...
      return {
        ...
        editingColumn,
        editingForm,
        handleCellClick, // Keep the function definition if needed elsewhere, but commented out the binding
        handleCellDbClick,
        saveEdit, // Keep if needed for dialog edit
        saveDialogEdit,
        getOptionsForColumn,
        getGroupColumnLabel,
        ...
        handleBatchSubmit,
        handleSaveEdit, // Keep if needed for dialog edit
  ...
  /* 历史记录样式 */
  .history-item {
    padding: 0; /* 移除内边距，因为 timeline item 已有 */
  ...
  :deep(.basic-info-section .el-descriptions__label) {
      width: 110px; /* 调整标签宽度 */
      text-align: right;
      color: #99a9bf;
  }
  :deep(.basic-info-section .el-descriptions__content) {
      min-width: 150px; /* 内容最小宽度 */
  }
  ...
  .sub-table-container {
    padding: 0; /* 移除内边距，因为 el-tabs__content 已有 */
    background-color: transparent;
  }
  ...
  .placeholder-content {
    ...
    align-items: center;
    justify-content: center;
    flex-direction: column; /* 让按钮在文字下方 */
    gap: 10px; /* 添加间距 */
  }
  ...
  .expense-summary-section .el-card {
    /* height: 100%; */
  }
  
  :deep(.expense-summary-section .el-card__header) {
      /* padding: 10px 15px; */
      /* background-color: #f5f7fa; */
      /* border-bottom: 1px solid #ebeef5; */
      /* font-weight: bold; */
  }
  :deep(.expense-summary-section .el-card__body) {
      /* padding: 15px; */
  }
